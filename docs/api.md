...
- **Documentos extra** (`unico_por_cliente: sí`, `forma: "documentos_extra"`): el resto de documentos opcionales que el cliente puede enviar (1099-INT/DIV/R/G/B/C/MISC/K/S/SA, 1098/1098-E/1098-T, SSA-1099, K-1 recibido, W-2G, 5498-SA, declaración del año anterior). A diferencia de la identidad del cliente, **no** aparecen proactivamente en `GET /api/clientes/{id}/pendientes` — el agente no le pregunta por ellos a todo cliente. Se consultan aparte, de forma reactiva, vía [`GET /api/catalogo/documentos-extra`](#catálogo-de-documentos-extra-get-apicatalogodocumentos-extratax_year), cuando el cliente menciona o sube un documento que no fue solicitado explícitamente. Misma mecánica de guardado que la identidad del cliente una vez identificados, ver [Cómo se guardan los datos únicos](#cómo-se-guardan-los-datos-únicos-por-cliente).
...
Estos 18 se envían con `forma: "documentos_extra"` cuando el cliente los entrega, y se guardan una sola vez por cliente igual que la identidad del cliente (ver [Cómo se guardan los datos únicos](#cómo-se-guardan-los-datos-únicos-por-cliente)) — pero, a diferencia de la identidad del cliente, **no** aparecen en `GET /api/clientes/{id}/pendientes`: el agente no le pregunta por ellos a todo cliente. Se consultan aparte vía [`GET /api/catalogo/documentos-extra`](#catálogo-de-documentos-extra-get-apicatalogodocumentos-extratax_year), reactivamente, cuando el cliente menciona o sube uno sin que se lo pidieran. Ninguno es obligatorio, así que ninguno cuenta para la completitud de una forma.

**Fase 5:** `form_1099_int`, `form_1099_div`, `form_1099_r`, `form_1099_g`, `form_1098` y `form_1098_e` son opcionales — no todo cliente los recibe (dependen de si tuvo intereses, dividendos, distribuciones de retiro, desempleo/reembolso estatal, o hipoteca/préstamo estudiantil ese año). Cada uno trae en su clave `revela` (ver [Catálogo de documentos extra](#catálogo-de-documentos-extra-get-apicatalogodocumentos-extratax_year)) a qué campo de `form_1040`/`schedule_c` alimenta, derivado de la matriz de trazabilidad GTS Form 1040 2025 — ver `RelacionesDocumentoCampoSeeder`.
...
- **`revelados`** (`array`, opcional): un item por cada campo que este documento ya resuelve. Cada item tiene la misma forma que un evento normal de tipo dato, sin `cliente_id`/`tax_year`/`modo` (se heredan del evento raíz — siempre es modo `"texto"`, un revelado nunca es otro documento):
  - `forma`, `campo`, `tipo_campo`, `tipo_dato`: los cuatro copiados TAL CUAL de ese mismo elemento de `revela` (ver [Qué falta por recolectar](#qué-falta-por-recolectar-get-apiclientesidpendientestax_year)) — nunca `"dato"`/`"number"` por default. `tipo_campo` normalmente es `"dato"`, pero puede ser `"mixto"` si el catálogo así lo tiene para ese campo destino (ej. `gastos_cuidado_dependientes`, que también acepta un documento en otro momento); nunca es `"documento"`, un revelado siempre se resuelve con texto. `tipo_dato` es el del campo CONTENEDOR, no el del subcampo (ej. `form_1040.ingresos` es `tipo_dato: "object"` en `revela` aunque el subcampo que se esté llenando, como `salarios`, sea en sí un número).
  - `contenido`: el valor, mismas reglas que el `contenido` raíz — cuando el destino es un subcampo de un campo `object`, solo necesita traer ese subcampo (ej. `{"salarios":"55665"}`), la API conserva los demás subcampos ya guardados por otro documento (ver `subcampo` abajo).
  - `subcampo` (opcional): cuando ese elemento de `revela` trae `subcampo` no nulo — se envía tanto si `acumulable` es `true` como `false`.
...
Cada item se valida contra el catálogo igual que el campo raíz — un `forma`/`campo` inexistente, un `tipo_campo`/`tipo_dato` que no coincide, o un `campo` que resulte ser de tipo `documento` en el catálogo, responden `422` con el error en `revelados.{i}.{campo}` (ej. `revelados.0.tipo_dato`). En una request `multipart/form-data` (necesaria por el `file` del documento), tanto `revelados` como cualquier `contenido` de tipo `object`/`array_string`/`array_object` pueden llegar como un string JSON en vez de un arreglo/objeto nativo — la API acepta ambas formas.
...
```json
{
  ...
  "estado": "recibido",
  "revela": [
    { "forma": "form_1040", "campo": "ingresos", "tipo_campo": "dato", "tipo_dato": "object", "subcampo": "salarios", "descripcion": "Box 1 (Wages, tips, other compensation) del W-2 es el salario total del cliente.", "acumulable": false },
    { "forma": "form_1040", "campo": "impuestos_retenidos", "tipo_campo": "dato", "tipo_dato": "number", "subcampo": null, "descripcion": "Box 2 (Federal income tax withheld) del W-2 suma directo a la retención federal total.", "acumulable": true }
  ]
}
```
...
- **`revela`** (mismo shape que en [Qué falta por recolectar](#qué-falta-por-recolectar-get-apiclientesidpendientestax_year)): se repite acá el `revela` que ya traía esta entrada en la última respuesta de `consultar_pendientes_cliente`, para que el agente no dependa de recordarlo de un turno anterior — encontrado en producción con modelos más pequeños (ej. gpt-5-mini) sobre prompts largos: el agente guardaba el documento principal pero nunca encadenaba las llamadas para los campos que ese documento revela, porque esa clave solo vivía en una respuesta 1-2 turnos atrás. Vacío (`[]`) para casi todo campo `tipo_campo: "dato"` y para cualquier campo sin relaciones confirmadas. `tipo_campo`/`tipo_dato` (agregados junto con `revelados`, ver más abajo) son los del campo DESTINO según el catálogo maestro — otro caso real encontrado en producción: sin estas dos claves, el agente adivinaba `tipo_campo: "dato"` y el `tipo_dato` del propio subcampo (ej. "number" para `salarios`) en vez de los reales del campo contenedor (`form_1040.ingresos` es `tipo_campo: "dato"` / `tipo_dato: "object"`, y `gastos_cuidado_dependientes` es `tipo_campo: "mixto"`), y la API rechazaba el `revelado` con 422 por no coincidir con el catálogo.
...
GET    /api/catalogo/documentos-extra?tax_year=              — catálogo de documentos extra, para consulta reactiva (tax_year obligatorio, no depende de un cliente)
```

`tax_year` en los endpoints de **lectura** (`GET`) es opcional: si se omite, usa el año fiscal actual configurado en la plataforma. En los que identifican/modifican un campo puntual (`campos/{campo}`, `marcar-revisado`, `formas`, `pendientes`, `catalogo/documentos-extra`) es **obligatorio** — ver [Año fiscal](#año-fiscal-taxyear).
...
### Catálogo de documentos extra (`GET /api/catalogo/documentos-extra?tax_year=`)

Consulta **reactiva**, no ligada a un cliente: a diferencia de `pendientes`, los 18 `documentos_extra` (ver [Catálogo maestro de campos](#catálogo-maestro-de-campos)) no se preguntan proactivamente a ningún cliente. El agente conversacional la llama solo cuando el cliente **menciona o sube, sin que se lo pidieran**, un documento que no reconoce (ej. "también tengo un 1099-K de una plataforma de pagos") — para confirmar que es un campo real del catálogo y obtener su `tipo_campo`/`tipo_dato`/`formatos_aceptados`/`revela` exactos antes de invocar `guardar_campo_cliente` con `forma: "documentos_extra"`. Requiere ability `clientes:read`. `tax_year` es obligatorio (sin default).

```bash
curl -s "https://tu-dominio/api/catalogo/documentos-extra?tax_year=2025" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Accept: application/json"
```
...
```json
{
  "tax_year": 2025,
  "documentos": [
    { "forma": "documentos_extra", "campo": "form_1099_int", "tipo_campo": "documento", "tipo_dato": null, "subcampos": null, "formatos_aceptados": ["pdf", "jpg", "jpeg", "png", "heic"], "obligatorio": false, "sensible": false, "revela": [
      { "forma": "form_1040", "campo": "ingresos", "tipo_campo": "dato", "tipo_dato": "object", "subcampo": "intereses_dividendos", "descripcion": "Casilla 1 (Interest income) del 1099-INT es interés gravable a incluir en ingresos.", "acumulable": true },
      { "forma": "form_1040", "campo": "impuestos_retenidos", "tipo_campo": "dato", "tipo_dato": "number", "subcampo": null, "descripcion": "Casilla 4 (Federal income tax withheld) del 1099-INT suma a la retención federal total.", "acumulable": true }
    ] },
    { "forma": "documentos_extra", "campo": "declaracion_anio_anterior", "tipo_campo": "documento", "tipo_dato": null, "subcampos": null, "formatos_aceptados": ["pdf"], "obligatorio": false, "sensible": false, "revela": [] }
  ]
}
```

Cada item usa el mismo shape que una entrada de `pendientes` (ver arriba), incluida `revela` — así el agente reutiliza exactamente la misma lógica de `RELACIONES DOCUMENTO→CAMPO` que ya aplica para lo que sí es proactivo. No filtra por lo que el cliente ya tenga cargado: siempre devuelve el catálogo completo de los 18, independientemente de si alguno ya fue guardado antes por otro camino (ej. un preparador lo cargó manualmente desde el panel).
