...
- [x] Cada campo con sus atributos: `tipo` (`documento`/`dato`/`mixto`), `tipo_dato`, `formatos_aceptados`, `subcampos`, `obligatorio`. → Se agregó además `sensible` (cifrado/enmascarado) y `unico_por_cliente` (dato de la persona, se guarda una sola vez bajo la forma `transversal`; ver nota 2026-07-28/29) — ninguno de los dos estaba en el spec original.
...
- [x] Idempotencia: mismo `cliente_id` + `forma` + `campo` sobrescribe, no duplica. → `unique(user_id, forma, campo)` + `updateOrCreate`. Para campos `unico_por_cliente` la `forma` de almacenamiento es siempre `transversal`, así que la unicidad efectiva es `(user_id, campo)` — no se duplican entre formas (ver nota 2026-07-28).
...
- [x] Tabla `campos_cliente` (user_id, forma, campo, tipo_campo, modo, valor_texto cifrado, documento_id, estado, source, actualizado_por). → `forma` vale una de las 10 formas **o** `transversal` para los campos `unico_por_cliente` (una sola fila por `(user_id, campo)`).
...
- **2026-07-28** — Se agregó el flag `unico_por_cliente` al catálogo (`catalogo_campos.unico_por_cliente`) para los datos que son de la **persona**, no de una forma. Estos se guardan una sola vez bajo la forma canónica `transversal` (`campos_cliente.forma = "transversal"`), sin importar en qué forma llegó el evento — antes se duplicaban por forma (ej. un SSN distinto en `form_1040` y `schedule_c`). Cambios: `TaxFieldCatalog::isUnicoPorCliente()`/`formaAlmacen()`, normalización de la forma de almacenamiento en `EventoRecoleccionService` (guardar/eliminar) y en los controladores de corrección/historial/reveal, completitud que cuenta los transversales para todas las formas y recálculo multi-forma, y `forma` acepta `transversal` además de las 10 `TaxForm`. También se quitó `dependientes` de `form_1040` (se solapaba con el transversal `info_dependientes`). Migraciones: `..._add_unico_por_cliente_to_catalogo_campos`, `..._marcar_documentos_basicos_unico_por_cliente` (consolidan duplicados existentes al valor más reciente; el historial se preserva).
- **2026-07-29** — Definición final de qué es único vs. por forma (confirmado con el usuario). **Únicos por cliente** (6): `identificacion_ssn_itin`, `info_conyuge`, `info_dependientes`, `w2`, `form_1099_nec`, `declaracion_anio_anterior`. **Por forma**: los documentos financieros dejan de ser transversales — `gastos_deducibles`, `activos_depreciacion` y `pl_balance_general` se **eliminan** (cada forma ya tiene su equivalente: `gastos`/`deducciones`, `activos`/`depreciacion`, `estados_financieros`/`balance_general`), y `estados_bancarios` pasa a ser campo propio de las formas de negocio (`schedule_c`, `schedule_e`, `schedule_f`, `form_1065`, `form_1120`, `form_1120_s`). Migración: `..._financieros_por_forma` (borra los 3 genéricos y sus datos —conserva historial— y crea `estados_bancarios` por forma). `1041` y `990` quedaron fuera de `estados_bancarios` (decisión abierta: se agregan si el negocio lo pide).
- **2026-07-29** — `POST /eventos` ahora acepta `forma: "transversal"` (además de las 10 formas) para emitir los datos únicos por cliente de forma semánticamente correcta, sin tener que atarlos a una forma. El servicio ya lo soportaba (recálculo multi-forma cuando no hay forma real); solo faltaba habilitarlo en `EventoRequest`. Enviar una forma real para un campo único también sigue funcionando (la API lo reubica bajo `transversal`); enviar `transversal` con un campo que pertenece a una forma da `422`. Ejemplos de `docs/api.md` actualizados a `transversal`.
